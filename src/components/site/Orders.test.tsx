...
describe("Orders", () => {
  ...
  it("opens WhatsApp with a pre-formatted order message", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Celular"), { target: { value: "(27) 99999-0000" } });
    fireEvent.click(screen.getByRole("combobox"));
    const [menuOption] = await screen.findAllByRole("option", { name: "Bolo inteiro" });
    fireEvent.click(menuOption);
    fireEvent.change(screen.getByLabelText("Data prevista"), { target: { value: "2026-05-20" } });

    fireEvent.click(screen.getByRole("button", { name: /Solicitar Orçamento Ágil via WhatsApp/i }));
